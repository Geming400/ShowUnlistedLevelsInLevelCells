...
class MyResetSavedInfosButtonSettingV3 : public SettingV3 {
public:
    // Once again implement the parse function
    static Result<std::shared_ptr<SettingV3>> parse(std::string const& key, std::string const& modID, matjson::Value const& json) {
        auto res = std::make_shared<MyResetSavedInfosButtonSettingV3>();
...
protected:
    ...
    bool init(std::shared_ptr<MyResetSavedInfosButtonSettingV3> setting, float width) {
...
public:
    ...
    // setting
    std::shared_ptr<MyResetSavedInfosButtonSettingV3> getSetting() const {
        return std::static_pointer_cast<MyResetSavedInfosButtonSettingV3>(SettingNodeV3::getSetting());
    }
};
...
// Register as before
$execute {
    (void)Mod::get()->registerCustomSettingType("reset-saved-infos-button", &MyResetSavedInfosButtonSettingV3::parse);
}
