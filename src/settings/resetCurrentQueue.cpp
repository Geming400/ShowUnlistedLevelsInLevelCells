...
class MyResetCurrentQueueButtonSettingV3 : public SettingV3 {
public:
    // Once again implement the parse function
    static Result<std::shared_ptr<SettingV3>> parse(std::string const& key, std::string const& modID, matjson::Value const& json) {
        auto res = std::make_shared<MyResetCurrentQueueButtonSettingV3>();
...
protected:
    ...
    bool init(std::shared_ptr<MyResetCurrentQueueButtonSettingV3> setting, float width) {
...
public:
    static MyResetCurrentQueueButtonSettingNodeV3* create(std::shared_ptr<MyResetCurrentQueueButtonSettingV3> setting, float width) {
    ...
    // setting
    std::shared_ptr<MyResetCurrentQueueButtonSettingV3> getSetting() const {
        return std::static_pointer_cast<MyResetCurrentQueueButtonSettingV3>(SettingNodeV3::getSetting());
    }
};

// Create node as before
SettingNodeV3* MyResetCurrentQueueButtonSettingV3::createNode(float width) {
    return MyResetCurrentQueueButtonSettingNodeV3::create(
        std::static_pointer_cast<MyResetCurrentQueueButtonSettingV3>(shared_from_this()),
        width
    );
}
...
// Register as before
$execute {
    (void)Mod::get()->registerCustomSettingType("reset-current-queue-button", &MyResetCurrentQueueButtonSettingV3::parse);
}
