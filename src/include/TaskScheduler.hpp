// repo: https://github.com/90th/LambdaTaskScheduler/blob/main/include/TaskScheduler.hpp
// I had to modify it a little bit to fix stupid build errors
...
class OneTimeTask : public taskScheduler::Task {
public:
...
class RecurringTask : public taskScheduler::Task {
public:
...
public:
    ...
    void addTask(std::unique_ptr<taskScheduler::Task> a_task) {
...
private:
    ...
    struct TaskEntry {
        std::unique_ptr<taskScheduler::Task> task;

        TaskEntry()
                : task(nullptr) {}

        explicit TaskEntry(std::unique_ptr<taskScheduler::Task> a_t)
                : task(std::move(a_t)) {}
    };
